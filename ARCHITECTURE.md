...
- Availability settings use a unique user/workspace row with mode, timezone, weekly JSON hours and revision; no row means Always. GET/PATCH interfaces use browser session/CSRF or Sanctum selected workspace, never arbitrary member IDs. Saves serialize initial creation by locking the user and enforce revisions. Export requests pin the originating workspace before queueing; old unpinned export jobs skip. The schedule evaluator is shared with AI but settings are independent.
