...
Conversation ownership/status actions use `ConversationActivityService`: a workspace-scoped conversation row lock and database transaction persist the state change and `messages.direction=system`, `type=event`, `sent_by=system` record together. Actor ID and name are snapshotted in `payload.activity`; no provider ID or `MessageSent`/`MessageReceived` event is created. Staff realtime uses `ConversationActivityCreated` on the private conversation channel only. Widget session/poll and `WidgetMessageCreated` expose only joined/resolved activity (transfer becomes joined) with actor name but no internal user ID. Activity does not change `last_message_at`, unread, inbox preview, AI history, notifications, provider sends, or billing. Staff mobile conversation and email-thread detail/history APIs include system activity records; conversation detail also exposes `joined_at`/`joined_user`. The Developer API continues to return only `in/out` messages. The Flutter repository remains unchanged, so native rendering/adoption is a separate app release. The customer SDK remains separately owned and is not released here.
