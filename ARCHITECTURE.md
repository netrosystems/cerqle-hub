...
Sanctum-authenticated mobile clients read workspace-wide omni-channel totals from `GET /api/v1/mobile/stats`. The endpoint aggregates all matching conversations without pagination and returns total, open, unread-message, unread-conversation, assigned, resolved, and per-channel counts. Email and SMS remain outside the omni-channel stats surface, matching the mobile inbox boundary.
