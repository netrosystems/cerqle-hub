# Cerqle Hub — UI/UX Design System & Strict Style Guidelines

This document defines the mandatory UI/UX standards, component specifications, color tokens, layout archetypes, and accessibility patterns for **Cerqle Hub**. All frontend components, page views, and design modifications must strictly adhere to these guidelines.

---

## 1. Typography & Font Hierarchy

### 1.1 Primary Font Family: Poppins
- **Mandatory Rule**: Cerqle Hub strictly uses **`Poppins`** (`'Poppins', sans-serif`) across all headings, body text, tables, modals, badges, and controls.
- **Font Stack**: `['Poppins', ...defaultTheme.fontFamily.sans]`. Multiple competing font families are forbidden.
...
## 2. Color Palette & The Cerqle Brand Restraint Rule

The visual identity of Cerqle is defined by its signature **Plum & Lilac** palette:

```text
Surface Canvas:  #FBF9FD   Surface Subtle: #F5EFF8   Card Surface: #FFFFFF
Primary Plum:    #3E2A49   Brand Lilac:    #8F5FA7   Cool Accent:  #45B6D6   Coral Danger: #f04e2e
```
...
{
  "brand": {
    "50": "#faf7fc",  "100": "#f1e8f5", "200": "#e3d5ea",
    "300": "#c7a3d7", "400": "#ad7ac2", "500": "#8f5fa7",
    "600": "#77488e", "700": "#603a72", "800": "#50305e",
    "900": "#3e2a49", "950": "#281a31"
  },
  "accent": {
    "50": "#f8fbfd",  "100": "#edf6fb", "500": "#45b6d6",
    "700": "#257b98", "900": "#23576b", "950": "#173747"
  },
  "coral": {
...
> **IMPORTANT**:
> - **Brand Lilac (`#8F5FA7` / `brand-600`)** is reserved for primary Call-To-Action buttons (e.g. "Save Changes", "Send Message", "Create Campaign"), active navigation highlights, and interactive toggles.
> - **Primary Plum (`#3E2A49` / `brand-900`)** is used for structured navigation contrast, sidebar themes, and dark accent surfaces.
> - Avoid excessive saturated color fills across large background areas; maintain crisp whitespace with light lavender-slate surfaces (`#FBF9FD`).
...
- **Initial Automations creation palette (2026-09-13, revised after audit)**: Show only SEND with four actions: Send WhatsApp, Send Template, Send Media and Quick Replies. No LISTEN, LOGIC, CONTACT, ENGAGE, COMMERCE or INTEGRATIONS creation options. Preserve stored nodes with a legacy warning. Preview sample inputs belong behind expandable Preview options. Instructions must mention click-to-add as well as dragging. Trigger selects the exact WABA/phone account; template choices belong to that sender. Show saved/unsaved state, node-specific validation errors and navigation-loss warnings. Activation saves the reviewed canvas; Preview is simulation-only. Run history shows waiting reason, next-check time and accessible pagination.
...
- Inbox Setup omits the generic Getting Started steps and external Resources panel, including its empty state. Keep connection controls, channel inventory and necessary webhook configuration.
- The WhatsApp connection drawer title is “Connect WhatsApp”; its English mode labels are “WhatsApp Business App” (coexistence) and “Connect WABA” (standard Cloud API).
- **WhatsApp connection modes (2026-09-15)**: Every client sees Connect WABA and WhatsApp Business App, a one-line mode description and expandable limitations. The global onboarding switch may disable Business App with a compact explanation, never hide the selector or restrict it by workspace. Number entry happens only in Meta; choosing coexistence prepares the server session automatically before the user clicks Continue with Meta (preserving popup activation). No duplicate phone field, acknowledgement checkbox or restart link. History/contact import stays off. Phone cards and inbox bubbles identify Business app connections and messages without inventing an agent identity.
...
- Conversation activity in Omni Inbox and Email Inbox uses a small centered, lightly outlined chip matching the staff inbox convention, not an agent/customer bubble, avatar, or delivery tick. The website widget keeps its plain centered text row without a chip; widget activity is silent and leaves the unread badge unchanged. Agent names are not bolded; text is escaped before rendering.
...
4. **Pagination Bar (`Pagination.jsx`)**:
   - Numbered buttons with active state in Brand Lilac (`bg-brand-600 text-white`).
