...
- Conversation activity in Omni Inbox, Email Inbox, and the website widget uses a small centered text row, not an agent/customer bubble, avatar, delivery tick, or raised white chip. Widget activity is silent and leaves the unread badge unchanged. Agent names use normal weight; text is escaped before rendering.
