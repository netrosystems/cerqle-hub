...
- **Unified login safeguards (2026-09-15)**: Existing accounts with confirmed 2FA must complete it across password, magic-link, social/Firebase and mobile login. Browser challenges expire after ten minutes; mobile returns `202 two_factor_required` before issuing a token. Inactive users/clients cannot retain operational access. Accounts without 2FA keep their existing login flow. Restricted developer tokens cannot escalate through token management or agent/mobile APIs. Lower-tier administrators cannot grant or edit beyond their authority; the owner's pre-release `view_clients` impersonation/plan-assignment exception is deliberately unchanged.
...
- **Conversation activity (2026-09-23)**: Assignment and actual joining are separate. A joined agent is recorded in `joined_user_id`; explicit Join opens the conversation. Administrators may always take over an active owner; an available staff member may take over only when the current owner is unavailable according to the dedicated workspace-member schedule. Notification availability remains independent. Resolve creates a customer-visible "resolved" entry. Staff timeline also records assignment, unassignment, transfer, leave, reopen, pending, and snooze. Automatic reopening on customer inbound creates no activity entry. Historic actions are not backfilled.
...
- **Separate website and customer SDK access (2026-09-23)**: One widget feeds one inbox, but website and customer mobile SDK use separate public keys and independent enable switches. Website embed/API access follows `widget_key` + `enabled`; SDK API access follows `sdk_widget_key` + `sdk_enabled` and does not require a browser origin. Existing website keys and old SDK builds remain compatible. New conversations retain their original `web_widget` or `customer_sdk` source for inbox icons and details; historical rows remain unclassified.
...
#### Capabilities
- **X publishing**: Super Admin configures/funds the shared X OAuth app; clients connect their own accounts. Internal network slug remains `twitter`. Standalone text (280 weighted characters), up to three uploaded JPEG/PNG images (5 MB each), or one uploaded GIF (15 MB) or H.264/AAC MP4 (500 MB/140 seconds) on its own — a Cerqle cap below X's four images, because X bills API use per request can publish immediately or on the existing scheduler. Text URLs are rejected, not silently stripped; other destinations may retain links through overrides. X uses existing social account/post plan limits without a separate allowance, surcharge or credits UI. Threads, engagement, analytics, Premium long posts and remote X edit/delete are deferred. Real delivery requires provider verification.
- **Email AI automation scope**: AI automatic replies for email apply to all connected mailboxes or only to chosen ones. Inbound mail is triaged first by free deterministic signals — the sender's declared headers, the shape of its address, and structural properties of the body — so newsletters, receipts, bounces and no-reply notifications are left for the team. When the signals do not decide, the mail is answered and flagged as unsure in the inbox rather than silently ignored.
- **Connected Accounts**: Connect Facebook Pages, Instagram Business Accounts, LinkedIn member profiles, and LinkedIn company pages via OAuth 2.0. The LinkedIn profile and page flows are separate networks with separate operator-configured credentials, because posting as a company page requires LinkedIn's Community Management API rather than the member scopes; one page authorisation connects every page that member administers.
...
- **Grounded Answering**: New Smart Bots default to Business only; existing bots remain General. Verified-only and General are also available. Business facts require published knowledge evidence in every mode. Incomplete Business-only profiles safely behave as Verified-only. Unsupported questions clarify once or offer human help, account/order-specific requests always offer human help, and the bot enabled switch remains an independent safety gate.
- **Knowledge Publishing**: Knowledge Bases retain URL, Sitemap, File and Text authoring. Indexing builds an unpublished full generation and atomically activates it only after success; failed builds leave the current generation available. Retrieval can blend semantic, lexical and typo-tolerant evidence and returns sanitized source citations. Live external research is not part of this release.
- **Any-language answering (2026-09-20)**: A customer may write in any language, including one nobody anticipated. Conversational moves are recognised by meaning rather than spelling, and the short fixed things the bot says are translated once per language and then reused for free, so no language list is shipped or maintained. A customer can reach a human by typing English, by picking the offer the bot made, or by asking in their own words. Channels without buttons receive the options as a numbered list, and a reply of "2" in any digit system selects one. When a handover lands outside working hours the customer is told the team has their message and when they are back, and the bot keeps helping until a person actually replies. Every turn that produces no bot reply records why. All switches ship disabled.
- **Grounded structured answers (2026-09-21)**: Replies are requested as a fixed JSON object and checked before sending. Any price, size or percentage that does not appear in the evidence is rejected, retried once, and if it still fails the customer receives the fallback and the credit is returned — clients pay for answers, not attempts. Choices come back as tappable options the server sanitises, and the reply names the customer's language so later free turns stay in it. Providers that cannot enforce a schema degrade to JSON mode and then plain text within the same charge.
