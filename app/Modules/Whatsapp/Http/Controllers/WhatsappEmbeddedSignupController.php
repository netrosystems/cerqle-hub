...
class WhatsappEmbeddedSignupController extends Controller
{
    public function store(Request $request): JsonResponse
    {
        ...
        $tokenRes = Http::get('https://graph.facebook.com/v25.0/oauth/access_token', $tokenParams);
        ...
        if (! $longTokenRes->successful() || ! $longTokenRes->json('access_token')) {
            // Facebook Login for Business may already issue the durable token
            // during the code exchange. A failed second exchange must not throw
            // away a valid authorization.
            Log::warning('WhatsApp embedded signup: long-lived token exchange skipped', [
                'workspace_id' => $workspaceId,
                'response' => $longTokenRes->json(),
            ]);
        ...
        $wabaId = ($validated['waba_id'] ?? null) ?: $this->discoverWabaId($accessToken, $meta);
        if (! $wabaId) {
            return response()->json([
                'message' => 'Meta authorized Cerqle, but no WhatsApp Business Account was included. Choose Edit Settings in Meta and select the WhatsApp account you want to connect.',
            ], 422);
        }
        ...
        if (! empty($validated['phone_number_id'])) {
            try {
                ...
                if ($this->registerNumber($validated['phone_number_id'], $accessToken, $wabaId, $registrationPin)) {
        ...
        $phoneWarning = $phoneCount === 0
            ? 'Meta connected the WhatsApp Business Account, but no phone number could be registered. Verify the number and its two-step-verification state, then use Sync from Meta.'
            : null;
    ...
    private function discoverWabaId(
        ...
        try {
            ...
            if (! $response->successful()) {
                Log::warning('WhatsApp embedded signup: token inspection failed', [
            ...
            Log::warning('WhatsApp embedded signup: WABA discovery failed', [
                'error' => $e->getMessage(),
            ]);
        }
    ...
    private function syncPhoneNumbers(
        ...
        foreach ($rows as $row) {
            ...
            $pin = (string) (($waba->credentials ?? [])['registration_pin'] ?? '');
            if ($pin === '' || ! $this->registerNumber((string) $row['id'], $syncToken, $waba->waba_id, $pin)) {
                continue;
            }
     ...
     */
    private function registerNumber(string $phoneNumberId, string $accessToken, string $wabaId, string $pin): bool
    {
        try {
            $result = CloudApiClient::registerPhoneNumber($phoneNumberId, $accessToken, $pin);
            ...
            if (! $result['success']) {
                ...
                // Reconnecting an already active number is idempotent from the
                // application's perspective. Meta may reject the second
                // /register call with an "already registered/connected" error;
                // that must not make us discard an otherwise valid phone row.
                if (str_contains($errorMessage, 'already registered')
                    || str_contains($errorMessage, 'already connected')) {
                    return true;
                }
