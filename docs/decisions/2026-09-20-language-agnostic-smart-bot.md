...
Still outstanding at the time of writing: the structured JSON contract, grounding validation with refunds, starter questions, exact wording and answer caches. `ai_kb_chunks` still carries no `workspace_id`; retrieval now verifies knowledge-base ownership at the boundary instead, and the column remains separate work.
