...
The backend adds `POST /api/v1/mobile/conversations/{uuid}/{join|leave|takeover}` for future staff clients. Join/leave are idempotent; an active join by another agent returns 409, and takeover of an active join requires a client administrator. Existing assignment and status routes retain their request/response shapes and now record staff activity. Mobile conversation and email-thread detail/history include `direction=system`, `type=event` activity records; conversation detail also exposes `joined_at` plus `joined_user`. Inbox preview/order/unread still use only content messages. Web, mobile conversation, and mobile email replies require the authenticated user to be the current joined agent; conflicts return 409 with nullable `joined_user`. Ownership transitions and outbound reply creation share the conversation AI/reply lock so a concurrent leave, takeover, or resolve cannot permit a stale send. Resolving clears assignment and joined ownership; a later genuine inbound customer message still reopens the conversation through the existing channel driver flow. The native Cerqle Agent repository remains unchanged, so mobile UI adoption of activity rows and join controls is a separate app release; do not claim it has shipped.
