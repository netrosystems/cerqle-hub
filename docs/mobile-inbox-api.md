...
The backend adds `POST /api/v1/mobile/conversations/{uuid}/{join|leave|takeover}` for future staff clients. Join/leave are idempotent; an active join by another agent returns 409. Join opens the conversation. Administrators may always take over; staff may take over only when their dedicated team schedule is available and the current owner's is unavailable. `GET /api/v1/mobile/team/availability` and `PUT /api/v1/mobile/team/{member}/availability` expose administrator-managed schedules; missing or disabled schedules mean always available and notification availability is independent. Existing assignment and status routes retain their request/response shapes and now record staff activity. Mobile conversation and email-thread detail/history include `direction=system`, `type=event` activity records; conversation/thread payloads expose `joined_at`, `joined_user`, and actor-specific `can_takeover`. Inbox preview/order/unread still use only content messages. Web, mobile conversation, and mobile email replies require the authenticated user to be the current joined agent; conflicts return 409 with nullable `joined_user`. Ownership transitions and outbound reply creation share the conversation AI/reply lock so a concurrent leave, takeover, or resolve cannot permit a stale send. Resolving clears assignment and joined ownership; a later genuine inbound customer message still reopens the conversation through the existing channel driver flow. The native Cerqle Agent repository remains unchanged, so mobile UI adoption of activity rows and join controls is a separate app release; do not claim it has shipped.
