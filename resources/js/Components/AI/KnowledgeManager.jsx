...
import { ArrowLeft, Plus, Pencil, RefreshCw, Trash2, Globe, FileText, Type, X, Upload, CheckCircle2, Clock, Zap, AlertCircle, HelpCircle } from 'lucide-react';
...
/**
 * The real knowledge screen: sources, uploads, indexing state, business
 * profile. Extracted from the knowledge base page so the Smart Bot can show
 * the genuine thing rather than a thinner copy of it that would drift.
 *
 * `embedded` drops the page chrome — layout, title, back link — so it can sit
 * inside a step on another page.
 */
export default function KnowledgeManager({ kb, kbUploadMaxKb = 20480, kbUploadMaxMb = 20, kbAppUploadMaxMb = 20, embedded = false }) {
    ...
    const handleAdd = (e) => {
        ...
        router.post(route('client.ai.knowledge-bases.documents.add', kb.uuid), formData, {
            preserveScroll: true,
            onSuccess: () => { reset(); setShowAdd(false); setProcessing(false); },
    ...
    return (
        <Shell>
                {/* Adding a document must always be reachable, embedded or not —
                    hiding this whole row is what left the step with no way to
                    add a second source. Only the page chrome is conditional. */}
                ...
                {/* The Smart Bot page owns business details as its own step, so
                    showing them again here would be two places to edit one thing. */}
                <div className={`${embedded ? 'hidden ' : ''}rounded-xl border px-4 py-3 ${kb.profile_complete ? 'border-neutral-200 bg-white dark:border-neutral-700 dark:bg-neutral-900' : 'border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-950/20'}`}>
                        ...
                        <table className="min-w-full divide-y divide-neutral-100 dark:divide-neutral-800 text-sm">
                            ...
                            <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
                                {kb.documents.map(doc => {
                                    const { icon: TypeIcon, labelKey: typeLabelKey } = DISPLAY_SOURCE_TYPES[doc.source_type] ?? DISPLAY_SOURCE_TYPES.file;
                                ...
                                >
                                    ...
                                    {data.file ? (
                                        ...
                                        <>
                                            ...
                                            {Number(kbUploadMaxMb) < Number(kbAppUploadMaxMb) && (
                                                <p className="text-xs text-amber-600 dark:text-amber-300 mt-1">Server upload limit is lower than Cerqle’s {kbAppUploadMaxMb} MB file limit.</p>
                                            )}
                                        </>
                                    )}
                                    {fileError && (
