...
describe('OPERATION_VISIBILITY windows', () => {
    it('uses five minutes for queued rows', () => {
        expect(OPERATION_VISIBILITY.queued).toBe(5 * 60 * 1000);
    });

    it('gives processing rows thirty minutes', () => {
        expect(OPERATION_VISIBILITY.processing).toBe(30 * 60 * 1000);
    });
...
describe('isLiveOperation', () => {
    ...
    it('hides a queued operation that has been stuck for more than five minutes', () => {
        expect(isLiveOperation({ status: 'queued', created_at: '2026-08-07T03:00:00Z' }, now)).toBe(false);
    });
    ...
    it('hides a processing operation that has been running for more than thirty minutes', () => {
        expect(isLiveOperation({ status: 'processing', created_at: '2026-08-07T02:30:00Z' }, now)).toBe(false);
    });
